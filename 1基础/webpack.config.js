...
module.exports = {
    ...
    mode: 'production', //模式  默认两种 production和 development  （开发模式代码没有压缩）
    entry: './src/index.js',
    output: {
        filename: 'bundle.js', //(hash显示8位)导出后的文件名,此处加了一个hash值，确保每次打包出来的文件名不同，避免缓存
    ...
    plugins: [ //数组  放着所有webpack插件
        ...
        new MiniCssExtractPlugin({ //将css单独抽离出来，生成main.css
            filename: 'main.css'
        })
    ],
    module: { //模块
